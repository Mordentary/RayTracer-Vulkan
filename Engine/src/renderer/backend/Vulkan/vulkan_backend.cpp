...
namespace SE
{
	...
	void VulkanBackend::init(const InitInfo& info)
	{
		initVulkan(info.window, info.enableValidation);
		initAllocator();
	}
	...
	void VulkanBackend::initVulkan(SDL_Window* window, bool enableValidation)
	{
		...
		if (khronosValidationAvailable && enableValidation) {
	...
	void VulkanBackend::initAllocator()
	{
